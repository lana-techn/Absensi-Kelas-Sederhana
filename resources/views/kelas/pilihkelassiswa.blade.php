        ...
        <div class="col-12 d-flex flex-wrap z-1 justify-content-center gap-md-5 p-md-5 overflow-y-scroll" id="boxcheck" style="min-height: 60vh; max-height: 73vh; ">
            @forelse ($data as $kls)
                <a href="{{ route('kelas.siswa', $kls->id) }}" class="text-decoration-none col-11 justify-content-start ps-4 ps-xl-0 h1 align-items-center col-xl-2 shadow text-xl-center text-right py-xl-5 rounded bg-warning d-flex flex-xl-column justify-xl-content-center align-items-center" style="height: fit-content">
                    ...
                    <label for="" class="h5 col-2 text-end pe-4 pe-xl-0 text-xl-center col-xl-12">{{ $kls->name }}</label>
                    <label for="" class="h5 col-8 text-end pe-4 pe-xl-0 text-xl-center col-xl-12">Jumlah Siswa : {{ $kls->jumlah_siswa }}/{{ $kls->rombel }}</label>
                </a>
            @empty
