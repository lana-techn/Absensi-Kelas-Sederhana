            ...
            <div class="d-flex flex-xl-wrap flex-column flex-xl-row gap-3 gap-xl-5 col-12 mt-5 align-items-center justify-content-center overflow-x-scroll" style="max-height: 73vh">
                @foreach ($data as $item)
                    @foreach ($item->pengajars as $newdata)
                        <a href="{{ route('kehadiran.edit',$newdata->id) }}" class="text-decoration-none col-11 justify-content-start ps-4 ps-xl-0 h1 align-items-center col-xl-2 shadow text-xl-center text-right py-xl-5 rounded bg-warning d-flex flex-xl-column justify-xl-content-center align-items-center" style="height: fit-content">
